...
const createUser = async (payload: CreateUser): Promise<ReturnUser> => {
  ...
  const createdUser: User = repository.create({
    ...payload,
  ...
  const result: ReturnUser = userSchemaReturn.parse({
    ...createdUser,
    registration_date: createdUser.registration_date.toISOString(),
  });
...
const readUsers = async (): Promise<ReturnUser[]> => {
  ...
  const formattedUsers = users.map((user) => ({
    ...
    contacts: user.contacts.map((contact) => ({
      ...
      registration_date: contact.registration_date.toISOString(),
    })),
  }));
...
const removeContact = async (
  ...
  const formattedContacts = user.contacts.map((contact) => ({
    ...
    email: contact.email,
    phone_number: contact.phone_number,
    registration_date: contact.registration_date.toISOString(), // Formate para string
  }));
